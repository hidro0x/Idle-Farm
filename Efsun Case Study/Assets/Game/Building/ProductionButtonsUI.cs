...
public class ProductionButtonsUI : MonoBehaviour
{
    ...
    {
        MobileTouchCamera.OnClickedOut.Subscribe(_ =>
        {
            Hide();
            _buildingObject = null;
        }).AddTo(_disposables);

        OnBuildingUIRequested.Subscribe(Init).AddTo(_disposables);
    }
    ...
    private void Init(BuildingObject buildingObject)
    {
        ...
        _buttonSubscriptions = _resourceController.Resources[buildingObject.Building.InputResource].CombineLatest(
            buildingObject.Building.CurrentTotalCapacity,
            (availableResource, currentCapacity) => buildingObject.Building.CanAddOrder(availableResource)).Subscribe(
            canProduce =>
            {
